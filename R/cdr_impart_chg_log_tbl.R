...
cdr_impart_chg_log_tbl <-function(
  ...
  chg_log_tbl <<- dplyr::tbl(db_conn_pool, chg_log_tbl_name) %>%
    dplyr::collect() %>% dplyr::arrange(dplyr::desc(WHEN_EDITED))
  if (db_conn_pool$objClass[[1]] == "SQLiteConnection") {
    chg_log_tbl <<- dplyr::mutate(chg_log_tbl, WHEN_EDITED = lubridate::ymd_hms(WHEN_EDITED,tz = "UTC"))
  }
